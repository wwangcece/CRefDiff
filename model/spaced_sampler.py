...
from img_utils.image import wavelet_reconstruction, adaptive_instance_normalization
from model.modules import cosine_attention_map
import os
...
class SpacedSampler:
    ...
    def __init__(
        self, model, schedule: str = "linear", var_type: str = "fixed_small"
    ) -> "SpacedSampler":
        self.model = model
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            ...
            # if given_x0 is not None:
                # alpha = cond["cos_sim_map_list"][0][0]
                # alpha = (cosine_attention_map(given_x0, pred_x0) + 1) / 2
                # alpha = (alpha**4).clip(0, 1)
                # print(f"Average confidence: {alpha.mean()}")
                # for i in range(alpha.shape[0]):
                #     save_name = os.path.splitext(os.path.basename(cond["path"][0][i]))[
                #         0
                #     ]
                ...
                #         exist_ok=True,
                #     )
                #     sim_map = alpha[i].unsqueeze(0).detach().cpu()  # [1, 1, h, w]
                ...
                #     # 保存
                #     save_path = os.path.join(save_path, f"{save_name}_{t[0]}.png")
                #     cv2.imwrite(save_path, sim_map_color)

                # alpha = F.interpolate(alpha, size=(given_x0.shape[2], given_x0.shape[3]))
                # alpha = 0.8
                # alpha = alpha if t[0] > 100 else 0
                # pred_x0 = pred_x0 + alpha * (given_x0 - pred_x0)
            model_mean, _, _ = self.q_posterior_mean_variance(
                x_start=pred_x0, x_t=x, t=index
            )
    ...
    def sample(
        ...
        if x_T is None:
            ...
            img = x_T
        if start_point == "ref":
            encoder_posterior = self.model.encode_first_stage(cond_img["hr"][0])
            ref_z = (
                self.model.get_first_stage_encoding(encoder_posterior)
                .detach()
                .to(device)
            )
            img = self.model.q_sample(
                x_start=ref_z,
                t=torch.full((b,), start_step, device=device, dtype=torch.long),
                noise=torch.randn_like(ref_z.float(), device=device),
            )
        else:
            ref_z = None
            ...
            if start_step is not None and step > start_step:
                ...
                if not return_each_step:
                    ...
                    img, mean = self.p_sample(
                        ...
                        uncond=uncond,
                        cond_fn=cond_fn,
                        return_mean=return_each_step,
                        given_x0=ref_z,
                    )
                    img_pixel = (self.model.decode_first_stage(mean) + 1) / 2
                    pixel_each_step.append(img_pixel)

        img_pixel = ((self.model.decode_first_stage(img) + 1) / 2).clip(0, 1)
        # apply color correction (borrowed from StableSR)
        if color_fix_type == "adain":
            img_pixel = adaptive_instance_normalization(
                img_pixel, (cond_img["lq"][0] + 1) / 2
            )
        elif color_fix_type == "wavelet":
            img_pixel = wavelet_reconstruction(img_pixel, (cond_img["lq"][0] + 1) / 2)
        else:
            assert (
                color_fix_type == "none"
            ), f"unexpected color fix type: {color_fix_type}"
